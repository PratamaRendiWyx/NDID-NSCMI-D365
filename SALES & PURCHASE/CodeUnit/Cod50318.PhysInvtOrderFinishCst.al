codeunit 50318 "Phys. Invt. Order-Finish - Cst"
{
...
#if not CLEAN24
        TempPhysInvtTrackingBuffer: Record "Invt. Order Tracking" temporary;
#endif
...
#if not CLEAN24
                        if not IsPackageTrackingEnabled then begin
                        ...
                        if PhysInvtOrderLine2.Find('-') then
                            repeat
                                ...
                                if PhysInvtOrderLine2."Quantity (Base)" <> 0 then begin
                                    ...
                                    if not IsHandled then
#if not CLEAN24
                                            if not IsPackageTrackingEnabled then
                                            CreateReservEntries(
    ...
    var
        ExpPhysInvtTracking: Record "Exp. Invt. Order Tracking";
    begin
        PhysInvtRecordLine.Reset();
        PhysInvtRecordLine.SetCurrentKey("Order No.", "Order Line No.");
    ...
    local procedure IsPackageTrackingEnabled(): Boolean
    var
        InvtSetup: Record "Inventory Setup";
    begin
        InvtSetup.Get();
        if InvtSetup."Ignore Package No. for Exp.Qty" then
            exit(false);
        exit(true);
    end;
    ...
    local procedure OnAfterCalcQtyToTransfer(var TempPhysInvtTrackingBuffer: Record "Invt. Order Tracking" temporary; AllBufferLines: Boolean; MaxQtyToTransfer: Decimal; var QtyToTransfer: Decimal)
    begin
    end;
#endif
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCodeOnAfterUpdateFromPhysInvtRecordLine(var PhysInvtTracking: Record "Invt. Order Tracking"; PhysInvtRecordLine: Record "Phys. Invt. Record Line")
    begin
    end;
#endif
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCreateReservEntriesOnBeforeInsert(var ReservationEntry: Record "Reservation Entry"; PhysInvtTracking: Record "Invt. Order Tracking"; PhysInvtOrderHeader: Record "Phys. Invt. Order Header"; PhysInvtOrderLine: Record "Phys. Invt. Order Line")
    begin
    end;
#endif
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCreateTrackingBufferLinesFromPhysInvtRecordLine(var TempPhysInvtTracking: Record "Invt. Order Tracking" temporary; PhysInvtRecordLine: Record "Phys. Invt. Record Line")
    begin
    end;
#endif
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCreateTrackingBufferLinesFromExpPhysInvtTracking(var TempPhysInvtTracking: Record "Invt. Order Tracking" temporary; ExpPhysInvtTracking: Record "Exp. Invt. Order Tracking")
    begin
    end;
#endif
    ...
    [IntegrationEvent(false, false)]
    local procedure OnUpdateBufferFromItemLedgerEntriesOnAfterUpdateExpectedQty(var PhysInvtTracking: Record "Invt. Order Tracking"; ItemLedgerEntry: Record "Item Ledger Entry")
    begin
    end;
#endif
