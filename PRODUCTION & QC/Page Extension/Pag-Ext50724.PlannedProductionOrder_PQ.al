pageextension 50724 PlannedProductionOrder_PQ extends "Planned Production Order"
{
    layout
    {
        modify("Source No.")
        ...
        addafter(Quantity)
        {
            ...
            field("Vacum Cycle"; Rec."Vacum Cycle")
            {
                Enabled = glbRestric;
                ApplicationArea = All;
                TableRelation = "Work Shift Vacum Cycle".Cycle where("Shift Code" = field(Shift));
            }
        ...
        {
            field("AMCreation Date"; Rec."Creation Date")
            {
    ...
    actions
    {
        ...
        {
            action("Split Production Order")
            {
                ApplicationArea = All;
                Caption = 'Split Production Order';
                Image = Split;
                ToolTip = 'Split Production Order Weekly to Daily';
                Promoted = true;
                ...
                trigger OnAction()
                var
                    SplitproductionOrder: Report SplitProductionOrder_PQ;
                begin
                    SplitproductionOrder.UseRequestPage(true);
                    SplitproductionOrder.setParameter(Rec);
