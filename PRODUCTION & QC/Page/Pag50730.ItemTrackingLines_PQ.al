page 50730 ItemTrackingLines_PQ
{
        ...
        area(content)
        {
            group(Control59)
            {
                ...
                fixed(Control1903651101)
                {
                    ...
                    group("Item Tracking IsFilter")
                    {
                        Caption = 'Item Tracking';
                        Visible = IsFilter;
                        field(Text020IsFilter; Text020)
                        {
                            ApplicationArea = ItemTracking;
                            Visible = false;
                            ShowCaption = false;
                        }
                        field(Quantity_ItemTrackingIsFilter; Rec."Quantity (Base)")
                        {
                            ApplicationArea = ItemTracking;
                            Caption = 'Quantity';
                            DecimalPlaces = 0 : 5;
                        ...
                        field(Handle2IsFilter; Rec."Qty. to Handle (Base)")
                        {
                            ApplicationArea = ItemTracking;
                            Caption = 'Qty. to Handle';
                            DecimalPlaces = 0 : 5;
                        ...
                        field(Invoice2IsFilter; Rec."Qty. to Invoice (Base)")
                        {
                            ApplicationArea = ItemTracking;
                            Caption = 'Qty. to Invoice';
                            DecimalPlaces = 0 : 5;
    ...
    begin
        ...
        if IsFilter then begin
            if Rec."Lot No." <> '' then
                Rec.SetFilter("Lot No.", LotNo);
            if Rec."Serial No." <> '' then
                Rec.SetFilter("Serial No.", SerialNo);

            if Rec.FindFirst() then begin
                SourceQuantityArray[1] := Rec."Quantity (Base)";
                SourceQuantityArray[2] := Rec."Qty. to Handle (Base)";
                SourceQuantityArray[3] := Rec."Qty. to Invoice (Base)";
                SourceQuantityArray[4] := Rec."Quantity Handled (Base)";
                SourceQuantityArray[5] := Rec."Quantity Invoiced (Base)";
            end;
    ...
    procedure AMGetLotSerialNo(NewLotNo: Code[50]; NewSerialNo: Code[50]; NewIsFilter: Boolean)
    begin
        LotNo := NewLotNo;
    ...
    procedure AMGetIsEmpty(NewIsEmpty: Boolean)
    begin
        IsEmpty2 := NewIsEmpty;
    end;
    ...
    begin
        ...
        SetControls(Controls::Reclass, CurrentRunMode = CurrentRunMode::Reclass);
    ...
    local procedure AssignSerialNoBatch(QtyToCreate: Integer; CreateLotNo: Boolean; CreateSNInfo: Boolean)
    var
        i: Integer;
    begin
        if QtyToCreate <= 0 then
    ...
    begin
        ...
        Rec.Validate("Quantity Invoiced (Base)", 0);
        AssignNewLotNo();
        OnAfterAssignNewTrackingNo(Rec, xRec, Rec.FieldNo(Rec."Lot No."));
        Rec."Qty. per Unit of Measure" := QtyPerUOM;
