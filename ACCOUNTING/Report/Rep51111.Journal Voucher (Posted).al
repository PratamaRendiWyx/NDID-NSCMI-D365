report 51111 "Journal Voucher (Posted)"
{
    DefaultLayout = RDLC;
    RDLCLayout = './ReportDesign/Journal Voucher (Posted).rdlc';
    ...
    dataset
    {
        dataitem(GLHeader; "Posted Gen. Journal Line")
        {
            ...
            column(G_L_Register_No_; format("G/L Register No.")) { }
            ...
            column(prepareCheck; CheckPrepareBy())
            {
            ...
            dataitem("Posted Gen. Journal Line"; "Posted Gen. Journal Line")
            {
                ...
                begin
                    ...
                    if (StrLen("Posted Gen. Journal Line".Description) > 46) OR (StrLen("Posted Gen. Journal Line"."Account Name") > 36) then
                        LineCount += 2
                    else
                        LineCount += 1;
                    //-
                    ...
                    if DocNo <> "Document No." then begin
                        ...
                        if PostedGenJournal.FindSet then begin
                            repeat
                                ...
                                //-Balancing Account
                                if PostedGenJournal."Bal. Account No." <> '' then begin
                                    if "Credit Amount" <> 0 then
                                        TotalDebitAmt += PostedGenJournal."Credit Amount";
                                    if "Debit Amount" <> 0 then
                                        TotalCreditAmt += PostedGenJournal."Debit Amount";
                                end;
                                //-
            ...
            dataitem("Posted Gen. Journal Line2"; "Posted Gen. Journal Line")
            {
                DataItemLink = "Document No." = FIELD("Document No.");
                trigger OnPreDataItem()
                var
                ...
                begin
                    ...
                    if "Debit Amount" <> 0 then
                        tempTB."Credit Amount" := "Debit Amount";
                    TotalDebitAmt += Abs(tempTB.Amount);
    ...
    begin
        Clear(GlRegister);
        GlRegister.SetRange("No.", GLHeader."G/L Register No.");
    ...
    local procedure CheckPrepareBy(): Text
    var
        Users: Record User;
    begin
        if SenderUser."Full Name" = 'ACC 1' then
            exit('Y');
        exit('N');
    end;
    ...
    begin
        ...
        if approvalEntry.Find('-') then begin
            SenderUser.SetRange("User Name", approvalEntry."Sender ID");
            if SenderUser.Find('-') then;
