...
report 51100 "Inventory Aging Detail Old"
{

    DefaultLayout = RDLC;
    RDLCLayout = './ReportDesign/Inventory Aging Detail.rdlc';
    ...
    dataset
    {
        dataitem(InventoryAgingTempFT; "Inventory Aging Temp FT")
        {
            ...
            var
                myInt: Integer;
                checkAgingQty: Decimal;
            begin
                //Check entry type transfer
    ...
    begin
        ...
        ILE.SetCurrentKey("Item No.", "Posting Date", "Entry Type", "Document No.", "Lot No.", "Document Type", Quantity);
        ILE.Ascending;
        if ILE.FindSet() then begin
            repeat
                if NOT (ILE."Document Type" IN [ILE."Document Type"::"Transfer Receipt"]) then begin
                    CurrData := ILE."Item No." + '#' + Format(ILE."Posting Date", 0, '<Day,2> <Month Text,3> <Year4>') + '#' + Format(ILE."Entry Type") + '#'
                                + ILE."Document No." + '#' + ILE."Lot No." + '#' + Format(ILE."Document Type") + Format(ILE.Quantity);
                    if TempData <> CurrData then begin
                        ...
                        if itemApplicationEntry.Find('-') then
                            ...
                            ILE1.SetRange("Entry No.", -1);

                        //matching check correction (exlude transaction)
                        if checkApplyCorrection(ILE."Entry No.") then
                            ILE1.SetRange("Entry No.", -1);
                        //-

                        //Check correction base on outbound entry 
                        if CheckUndoProcessBaseOnOutboundEntry(ILE."Entry No.") then begin
                            ILE1.SetRange("Entry No.", -1);
                        end;
                        //-
                        ...
                        if ILE1.FindSet() then begin
                            ...
                            InventoryAgingTempFT.Balance := ILE.Quantity;
                            ...
                            if AgingInt <= 90 then
                                InventoryAgingTempFT.Aging1 := ILE.Quantity;
                            if (AgingInt > 90) AND (AgingInt <= 180) then
                                InventoryAgingTempFT.Aging2 := ILE.Quantity;
                            if (AgingInt > 180) AND (AgingInt <= 270) then
                                InventoryAgingTempFT.Aging3 := ILE.Quantity;
                            if (AgingInt > 270) AND (AgingInt <= 360) then
                                InventoryAgingTempFT.Aging4 := ILE.Quantity;
                            if AgingInt > 360 then
                                InventoryAgingTempFT.Aging5 := ILE.Quantity;
